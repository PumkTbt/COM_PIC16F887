#line 1 "E:/Document/Giao tiep DKTBNV/19486901_TranBaTy_GTTBNV_DHDTMT15B/Baitap COM/BaitapCOM_NC_Finally/MikroC/PIC_NC_Finally.c"
...
void interrupt(void){
 ...
 if (strncmp(strstr(buf_string_receive,led_on_code),led_on_code, 5 ) == 0){
 LATE.LATE1 =  1 ;
 UART1_Write_Text("@Lle_on&");
 }
 else if (strncmp(strstr(buf_string_receive,led_off_code),led_off_code, 5 ) == 0){
 LATE.LATE1 =  0 ;
 UART1_Write_Text("@Lle_of&");
 }
 else if (strncmp(strstr(buf_string_receive,led_on_code2),led_on_code2, 5 ) == 0){
 LATE.LATE0 =  1 ;
 UART1_Write_Text("@Lleon2&");
 }
 else if (strncmp(strstr(buf_string_receive,led_off_code2),led_off_code2, 5 ) == 0){
 LATE.LATE0 =  0 ;
